...
export default function CrearNewsletterPage() {
  const router = useRouter();
  const [form, setForm] = useState<any>({ nombre_newsletter: "", edicion: "", numero_publicacion: "", cuenta_id: "", contenido_id: "", link: "", estado: "Pendiente" });
  ...
  const create = async () => {
    try {
      ...
      router.push(`/dashboard/produccion/publicaciones/newsletter/${created.id_newsletter}`);
    } catch (error: any) {
      setError(error?.message || "No se ha podido crear el newsletter.");
    }
  };

  return (
    <div className="flex min-h-screen w-full flex-col bg-gray-200 text-gray-700">
      <MiddleNav tituloprincipal="Crear newsletter" />
      <div className="min-h-screen w-full bg-gray-100 px-12 py-10">
        ...
        <section className="bg-white p-6 shadow-sm">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {["nombre_newsletter", "edicion", "numero_publicacion", "cuenta_id", "contenido_id", "link", "estado"].map((field) => (
              <label key={field} className="text-sm">
                <span className="mb-1 block text-xs font-semibold uppercase text-gray-500">{field === "numero_publicacion" ? "numero" : field}</span>

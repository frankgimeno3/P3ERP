...
export default function ReviewDetail({ params }: { params: Promise<{ id_linea_banco: string }> }) {
  ...
  return <main className="min-h-screen bg-gray-100 p-6 lg:p-12"><div className="mx-auto max-w-6xl">{error ? <p role="alert">{error}</p> : line ? <BankReviewWizard key={id_linea_banco} lines={[line]} all={all} onSaved={() => router.push('/dashboard/direccion/bancos/extractos/revision')} onClose={() => router.push('/dashboard/direccion/bancos/extractos/revision')} /> : <p>Cargando revisión…</p>}</div></main>;
}

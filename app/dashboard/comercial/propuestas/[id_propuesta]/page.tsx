...
export default function ResumenPropuesta({ params }: { params: Promise<{ id_propuesta: string }> }) {
  ...
  async function setEstado(estado: string, rejectOtherPending = false) {
    ...
    try {
      ...
      if (/aceptad|rechazad/i.test(estado)) setTrackingTasks([]);
      setStatusOpen(false);
  ...
  return <div className="min-h-screen bg-gray-100 text-gray-600">
    ...
    {picker && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-label={picker === "cuenta" ? "Modificar cuenta" : "Modificar contacto"}><div className="max-h-[85vh] w-full max-w-4xl overflow-hidden rounded-xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><h2 className="text-lg font-semibold text-blue-950">{picker === "cuenta" ? "Seleccionar cuenta" : "Seleccionar contacto"}</h2><button type="button" aria-label="Cerrar modal" onClick={() => setPicker(null)} className="cursor-pointer rounded px-2 py-1 text-xl hover:bg-gray-100">×</button></div><div className="p-5"><input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Filtrar por cualquier campo..." className="mb-4 w-full rounded border p-3" /><div className="max-h-[55vh] overflow-auto rounded border"><table className="min-w-full text-sm"><thead className="sticky top-0 bg-slate-800 text-white"><tr>{picker === "cuenta" ? <><th className="p-3 text-left">Código</th><th className="p-3 text-left">Cuenta</th><th className="p-3 text-left">País</th><th className="p-3 text-left">Correo</th></> : <><th className="p-3 text-left">Nombre</th><th className="p-3 text-left">Correo</th><th className="p-3 text-left">Teléfono</th></>}</tr></thead><tbody>{filteredRows.map((item) => <tr key={picker === "cuenta" ? item.id_cuenta : item.id_contacto} onClick={() => { if (picker === "cuenta") setDraft((current: any) => ({ ...current, id_cuenta_propuesta: item.id_cuenta, cuenta: item, id_contacto_propuesta: "", contacto: null })); else setDraft((current: any) => ({ ...current, id_contacto_propuesta: item.id_contacto, contacto: item, contacto_personalizado: null })); setPicker(null); }} className="cursor-pointer border-t transition hover:bg-blue-50">{picker === "cuenta" ? <><td className="p-3">{item.id_cuenta}</td><td className="p-3 font-medium">{item.nombre_empresa}</td><td className="p-3">{item.pais_cuenta || "—"}</td><td className="p-3">{item.correo_principal || "—"}</td></> : <><td className="p-3 font-medium">{item.nombre_completo_contacto}</td><td className="p-3">{item.email_contacto || "—"}</td><td className="p-3">{item.telefono_contacto || "—"}</td></>}</tr>)}</tbody></table></div></div></div></div>}
  </div>;
}

...
const comentariosContactos: { contactosConComentarios: any[] } = { contactosConComentarios: [] };
const agentes: any[] = [];
...
const FichaContacto: FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchContacto = async () => {
      ...
      const contacto = contactos.find((c: InterfazContacto) => c.id_contacto === contactoId);
    ...
    fetchContacto().catch((error) => {
      console.error("Error fetching contacto:", error);
      setContactoEditable(undefined);
    });
  }, [contactoId]);
  ...
  useEffect(() => {
    ...
    const comentariosFormateados = contactoComentarios.array_comentarios_contacto.map((c: any) => {

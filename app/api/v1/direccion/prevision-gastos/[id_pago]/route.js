...
export async function GET(_request, { params }) {
  ...
  const { rows } = await getPgPool().query("SELECT pg.*,p.nombre_proveedor FROM tesoreria_pagos_previstos pg LEFT JOIN administracion_proveedores p USING(id_proveedor) WHERE id_pago=$1", [id]);
...
export async function PUT(request, { params }) {
  try {
    ...
    const { rows } = await getPgPool().query(`UPDATE tesoreria_pagos_previstos SET id_proveedor=$1,id_factura_proveedor=$2,fecha_pago=$3,bi_pago=$4,total_pago=$5,forma_pago=$6,cuenta_pago=$7,nombre_planificacion=$8,descripcion_planificacion=$9,comentarios=$10,updated_at=NOW() WHERE id_pago=$11 RETURNING *`,

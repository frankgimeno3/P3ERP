...
try {
  if (!process.argv.includes('--apply')) {
    ...
    console.log(JSON.stringify(supplierMergePlan(rows).map(g => ({ proveedor:g.name, conservar:g.target.id_proveedor, unificar:g.sources.map(s => s.nombre_proveedor) })),null,2));
  } else {
    ...
    const result = await mergeSuppliers(client);
    await client.query(fs.readFileSync('database/migrations/20260905_0004_proveedores_nombre_unico.sql','utf8'));

...
try {
  if (!process.argv.includes('--apply')) {
    const { rows } = await client.query('SELECT * FROM administracion_proveedores');
    ...
    await client.query(readLegacyMigrationSql('database/migrations/20260905_0003_proveedores_tickets_tarjetas.sql'));
    const result = await mergeSuppliers(client, onlyNames);
    if (!onlyNames) await client.query(readLegacyMigrationSql('database/migrations/20260905_0004_proveedores_nombre_unico.sql'));

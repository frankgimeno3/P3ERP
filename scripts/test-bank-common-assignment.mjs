...
async function run({ line = {}, linked = false, entity = true, action = 'assign', entityType = 'nomina', existingCharge = true, extra = {} } = {}) {
  ...
  const db = { release() {}, async query(sql, values) {
    ...
    if (sql.startsWith('SELECT * FROM lineas_bancos')) return { rows: [{ id_linea_banco: 'bank-1', importe: -1000, ...line }] };
    if (sql.startsWith('SELECT id_transferencia')) return { rows: linked ? [{ id_transferencia: 'bank-1' }] : [] };
    if (sql.startsWith('SELECT * FROM cargos_recurrentes')) return { rows: existingCharge ? [{ id_cargo_recurrente: 1 }] : [] };
    if (sql.startsWith('INSERT INTO cargos_recurrentes')) return { rows: [{ id_cargo_recurrente: 2 }] };
    return { rows: [] };
  } };
...
assert.equal(result.response.status, 200);
assert.equal(result.calls.filter(c => c.sql.startsWith('INSERT INTO cargos_recurrentes')).length, 1);
assert(result.calls.some(c => c.sql.startsWith('UPDATE')));

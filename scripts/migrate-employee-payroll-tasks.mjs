...
try {
  ...
  const result=await db.query(`SELECT (SELECT count(*)::int FROM laboral_empleados_en_nomina) AS fichas_nomina,
    (SELECT count(*)::int FROM tesoreria_cargos_recurrentes WHERE tipo_cargo='nomina') AS cargos_nomina,
    (SELECT count(*)::int FROM laboral_tareas_empleado) AS tareas`);

...
try {
  ...
  const { rows } = await db.query("SELECT e.*,inicio::text AS inicio,fin::text AS fin FROM eventos_calendario_laboral e WHERE tipo='vacaciones' FOR UPDATE");
  ...
  if (apply) {
    ...
    for (const { event, ranges } of changes) {
      ...
      if (!ranges.length) await db.query('DELETE FROM eventos_calendario_laboral WHERE id=$1', [event.id]);
      for (let i = 0; i < ranges.length; i++) {
        const [inicio, fin] = ranges[i];
        if (i === 0) await db.query('UPDATE eventos_calendario_laboral SET inicio=$1,fin=$2 WHERE id=$3', [inicio,fin,event.id]);
        else await db.query('INSERT INTO eventos_calendario_laboral(id,anio,tipo,titulo,inicio,fin,comentarios,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8)', [randomUUID(),event.anio,event.tipo,event.titulo,inicio,fin,event.comentarios,event.created_at]);
      }
    }

...
try {
  ...
  await db.query(`CREATE TABLE agentes_db(id_agente TEXT PRIMARY KEY,is_empleado_account BOOLEAN);
    ...
    INSERT INTO tesoreria_movimientos_bancarios(id_linea_banco,importe) VALUES('old',-100),('new',-120),('exception',-90),('mismatch',-80),('income',50);`);
  ...
  const current=async()=> (await db.query('SELECT * FROM tesoreria_cargos_recurrentes WHERE activo')).rows[0];
  ...
  assert.equal((await db.query("SELECT * FROM tesoreria_movimientos_bancarios WHERE id_linea_banco='old'")).rows[0].nomina_revision.importe_previsto,100);
  assert.equal((await db.query("SELECT * FROM tesoreria_movimientos_bancarios WHERE id_linea_banco='old'")).rows[0].estado_revision,true);
  ...
  assert.equal((await db.query("SELECT * FROM tesoreria_movimientos_bancarios WHERE id_linea_banco='exception'")).rows[0].nomina_revision.decision,'puntual');
  await assert.rejects(reviewPayrollMovement('mismatch',await payload('coincide')),error=>error.status===409);
  assert.equal((await db.query("SELECT estado_revision FROM tesoreria_movimientos_bancarios WHERE id_linea_banco='mismatch'")).rows[0].estado_revision,false);

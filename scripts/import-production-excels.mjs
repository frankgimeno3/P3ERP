...
try {
  ...
  await client.query(readLegacyMigrationSql('database/migrations/20260902_0004_produccion_excel_fields.sql'));
  await client.query("UPDATE produccion_contenidos SET hoja_prod=false WHERE ano_publicacion='2026'");
  for (const row of hoja) {
    ...
    await client.query(`INSERT INTO produccion_contenidos (id_contenido,id_agente,codigo_crm_hoja,cliente_hoja,id_contrato,publicacion_num_web,tipo_revista_servicio,especificaciones_contenido,anuncio_hoja,articulo_hoja,estado_contenido,factura_hoja,pagina_hoja,caduca_web,comentarios_hoja,hoja_prod,ano_publicacion)
  ...
  await client.query('TRUNCATE TABLE produccion_control_redaccion RESTART IDENTITY');
  for (const row of redaccion) {
    ...
    await client.query(`INSERT INTO produccion_control_redaccion (prioridad,donde_esta,empresa,titulo,estado,responsable_correccion,revista,espana_previsto_numero,latam_previsto_numero,especial_numero,hueco_previsto,paginas,estado_publicacion_vidrioperfil) VALUES (${values.map((_,i)=>`$${i+1}`).join(',')})`, values);
  }

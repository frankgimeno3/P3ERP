...
export async function getTickets(idProveedor = '', ambito = '') {
  ...
  const { rows } = await getPgPool().query(`SELECT t.id_ticket,t.fecha_ticket,t.id_proveedor,t.nombre_personalizado_proveedor,
    ...
    FROM tickets_db t LEFT JOIN proveedores_db p ON p.id_proveedor=t.id_proveedor
...
export async function createTicket(body, file) {
  ...
  return supplierTransaction(async db => {
    ...
    await db.query('SELECT id_proveedor FROM proveedores_db WHERE id_proveedor=$1 FOR KEY SHARE',[provider.id_proveedor]);
    let card = null;
    if (data.forma_pago === 'tarjeta') {
      ...
      const { rows } = await db.query("SELECT * FROM tarjetas WHERE estado='activa' AND ultimos_digitos=$1 AND lower(btrim(banco))=lower(btrim($2)) AND ($3::text IS NULL OR id_tarjeta=$3) FOR SHARE",[data.tarjeta_ultimos_digitos,data.tarjeta_banco,data.id_tarjeta || null]);
    ...
    const result = await db.query(`INSERT INTO tickets_db(fecha_ticket,id_proveedor,nombre_personalizado_proveedor,base_imponible,importe_total,forma_pago,documento_src,ambito,id_tarjeta,tarjeta_ultimos_digitos,tarjeta_banco,tarjeta_nombre,tarjeta_tipo,archivo_nombre,archivo_tipo,archivo_contenido)
    ...
    await db.query('UPDATE tickets_db SET documento_src=$1 WHERE id_ticket=$2',[`/api/v1/admin/tickets/${id}/archivo`,id]);
...
export async function getTicketFile(id) {
  ...
  const { rows } = await getPgPool().query('SELECT archivo_nombre,archivo_tipo,archivo_contenido FROM tickets_db WHERE id_ticket=$1',[id]);

...
export async function mergeSuppliers(db, onlyNames = null) {
  ...
  for (const group of plan) {
    ...
    for (const source of group.sources) {
      ...
      await db.query('INSERT INTO proveedores_unificados(id_anterior,id_proveedor,datos_anteriores,datos_destino_anteriores,referencias_anteriores) VALUES ($1,$2,$3,$4,$5)', [source.id_proveedor,target.id_proveedor,JSON.stringify(source),JSON.stringify(group.target),JSON.stringify(backup)]);
      await db.query('UPDATE proveedores_unificados SET id_proveedor=$1 WHERE id_proveedor=$2', [target.id_proveedor,source.id_proveedor]);
      await db.query('DELETE FROM proveedores_db WHERE id_proveedor=$1', [source.id_proveedor]);
    }
    if (group.target.nombre_proveedor !== group.name) {
      await db.query('INSERT INTO proveedores_unificados(id_anterior,id_proveedor,datos_anteriores,datos_destino_anteriores) VALUES ($1,$1,$2,$2) ON CONFLICT(id_anterior) DO NOTHING', [target.id_proveedor,JSON.stringify(group.target)]);
    }
    await db.query('UPDATE proveedores_db SET nombre_proveedor=$1,nombre_fiscal_proveedor=$2,vat_code=$3,pais_proveedor=$4,moneda_proveedor=$5,updated_at=NOW() WHERE id_proveedor=$6', [group.name,target.nombre_fiscal_proveedor,target.vat_code,target.pais_proveedor,target.moneda_proveedor,target.id_proveedor]);

...
export async function getTarjetas() { return (await getPgPool().query('SELECT * FROM tesoreria_tarjetas ORDER BY tipo,estado,lower(nombre),ultimos_digitos')).rows; }
export async function saveTarjeta(id, body) {
  ...
  const result = await getPgPool().query(id ? 'UPDATE tesoreria_tarjetas SET ultimos_digitos=$1,nombre=$2,banco=$3,tipo=$4,estado=$5,updated_at=NOW() WHERE id_tarjeta=$6 RETURNING *' : 'INSERT INTO tesoreria_tarjetas(ultimos_digitos,nombre,banco,tipo,estado,id_tarjeta) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',values);
  if (!result.rows[0]) throw new ProveedorError('Tarjeta no encontrada.',404);
  return result.rows[0];
}
export async function deleteTarjeta(id) {
  const result = await getPgPool().query('DELETE FROM tesoreria_tarjetas WHERE id_tarjeta=$1 RETURNING id_tarjeta',[id]);

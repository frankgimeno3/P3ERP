...
export async function getRevistaById(idRevista) {
  ...
  const { rows } = await pool.query(
    hasPlanillo ? `
      ...
      FROM revistas_db r
      LEFT JOIN publicaciones_db p ON p.revista_id = r.id_revista AND (p.tipo_publicacion = 'revista' OR p.tipo_publicacion = '')
      LEFT JOIN contenidos_revistas_db cr ON cr.revista_id = r.id_revista
      LEFT JOIN contenidos_db c ON c.id_contenido = cr.contenido_id
...
export async function updateRevista(idRevista, data = {}) {
  ...
  if (revistaSets.length) {
    ...
    await pool.query(
      `UPDATE revistas_db SET ${revistaSets.join(", ")}, updated_at = NOW() WHERE id_revista = $${revistaValues.length}`,
      revistaValues,
    );
  }
...
async function resolvePublicationId(idPublicacion, pool = getPgPool()) {
  const { rows } = await pool.query(
    `SELECT id_publicacion FROM publicaciones_db
...
export async function setNumeroPaginas(idPublicacion, requestedPages) {
  ...
  try {
    ...
    await client.query("UPDATE publicaciones_db SET num_paginas=$1,updated_at=NOW() WHERE id_publicacion=$2", [numPaginas, resolvedId]);
...
export async function updatePaginaPublicacion(idPublicacion, idPagina, data = {}) {
  ...
  if (nextContentId) {
    const contenido = await pool.query("SELECT id_cuenta FROM contenidos_db WHERE id_contenido=$1", [nextContentId]);
...
export async function assignContentToPages(idPublicacion, data = {}) {
  ...
  try {
    ...
    const content = await client.query("SELECT id_cuenta FROM contenidos_db WHERE id_contenido=$1", [data.id_contenido]);
...
export async function createRevista(data = {}) {
  ...
  const { rows } = await pool.query(
    `
      INSERT INTO revistas_db (id_revista, edicion, revista, publicacion, deadline_materiales, fecha_publicacion, especial, impresa_o_digital)
      VALUES ($1, $2, $3, '', '', '', $4, $5)

...
export async function getHorasJuan() { const { rows } = await getPgPool().query(`SELECT * FROM laboral_horas_colaboradores ORDER BY anio DESC, mes DESC, fecha DESC, id_horas_juan DESC`); return rows.map(normalize); }
export async function getHorasJuanById(id) { const { rows } = await getPgPool().query(`SELECT * FROM laboral_horas_colaboradores WHERE id_horas_juan = $1`, [id]); if (!rows[0]) return null; const { rows: sources } = await getPgPool().query(`SELECT id_horas_juan, mes, anio, tipo, saldo_pendiente, importe_anticipo, importe_ajuste, motivo_ajuste FROM laboral_horas_colaboradores WHERE compensado_en_id = $1 ORDER BY anio, mes, id_horas_juan`, [id]); return { ...normalize(rows[0]), compensaciones: sources.map(normalize) }; }
export async function getHorasJuanDebts() { const { rows } = await getPgPool().query(`SELECT * FROM laboral_horas_colaboradores WHERE saldo_pendiente <> 0 AND compensado_en_id IS NULL ORDER BY anio DESC, mes DESC, id_horas_juan DESC`); return rows.map(row => ({ ...normalize(row), deudor: Number(row.saldo_pendiente) > 0 ? 'Proporción 3 a Juan' : 'Juan a Proporción 3', importe_deuda: Math.abs(Number(row.saldo_pendiente)) })); }
async function getPending(client, nombre, mes, anio, tipo) {
  const { rows: informativas } = await client.query(`SELECT * FROM laboral_horas_colaboradores WHERE nombre = $1 AND tipo IN ('informativa', 'normal') AND saldo_pendiente <> 0 AND compensado_en_id IS NULL AND (anio < $3 OR (anio = $3 AND mes <= $2)) ORDER BY anio, mes, id_horas_juan FOR UPDATE`, [nombre, mes, anio]);
  const { rows: anticipos } = await client.query(`SELECT * FROM laboral_horas_colaboradores WHERE nombre = $1 AND tipo = 'anticipo' AND mes = $2 AND anio = $3 AND compensado_en_id IS NULL ORDER BY id_horas_juan FOR UPDATE`, [nombre, mes, anio]);
...
export async function createHorasJuan(data) {
  ...
  try {
    ...
    if (tipo === 'anticipo') {
      ...
      const { rows } = await client.query(`INSERT INTO laboral_horas_colaboradores (mes, anio, nombre, tipo, importe_anticipo, importe_pagar, fecha) VALUES ($1,$2,$3,'anticipo',$4,$4,$5) RETURNING *`, [mes, anio, nombre, importe, fecha]);
    ...
    if (sourceIds.length) await client.query(`UPDATE laboral_horas_colaboradores SET compensado_en_id = $1, updated_at = NOW() WHERE id_horas_juan = ANY($2::bigint[])`, [created.id_horas_juan, sourceIds]);
...
export async function getHorasJuanDeleteImpact(id) {
  ...
  const { rows } = await getPgPool().query(`WITH RECURSIVE cadena AS (SELECT id_horas_juan, mes, anio, tipo, compensado_en_id FROM laboral_horas_colaboradores WHERE id_horas_juan = $1 UNION ALL SELECT h.id_horas_juan, h.mes, h.anio, h.tipo, h.compensado_en_id FROM laboral_horas_colaboradores h JOIN cadena c ON h.id_horas_juan = c.compensado_en_id) SELECT id_horas_juan, mes, anio, tipo FROM cadena WHERE id_horas_juan <> $1 ORDER BY anio, mes, id_horas_juan`, [id]);
...
export async function deleteHorasJuan(id, confirmDependencies = false) {
  ...
  const { rowCount } = await getPgPool().query(`DELETE FROM laboral_horas_colaboradores WHERE id_horas_juan = ANY($1::bigint[])`, [ids]);

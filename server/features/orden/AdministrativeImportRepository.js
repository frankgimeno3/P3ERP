...
export async function importAdministrativeOrders(rows, actorId='', pool=getPgPool()) {
  ...
  try {
    ...
    for(const incoming of [...rows].sort((a,b)=>a.id_orden.localeCompare(b.id_orden))){
      const row={...incoming};
      const before=(await db.query('SELECT * FROM ordenes_db WHERE id_orden=$1 FOR UPDATE',[row.id_orden])).rows[0];
      if(!row.id_cuenta && row.datos_importacion?.cliente){
        const matches=(await db.query('SELECT id_cuenta FROM cuentas_db WHERE id_cuenta=$1 OR lower(btrim(nombre_empresa))=lower($1) OR lower(btrim(nombre_fiscal))=lower($1)',[row.datos_importacion.cliente])).rows;
        if(matches.length===1)row.id_cuenta=matches[0].id_cuenta;
      }
      if(row.id_cuenta && !(await db.query('SELECT 1 FROM cuentas_db WHERE id_cuenta=$1',[row.id_cuenta])).rowCount)incomeError('La cuenta '+row.id_cuenta+' de la orden '+row.id_orden+' no existe.');
      if(row.id_factura){
        const found=(await db.query('SELECT * FROM facturas_clientes_db WHERE id_factura_cliente=$1 OR numero_factura=$1',[row.id_factura])).rows;
        ...
        else row.id_factura=(await db.query("INSERT INTO facturas_clientes_db(id_factura_cliente,numero_factura,id_cuenta,estado) VALUES($1,$2,$3,'en proceso') RETURNING id_factura_cliente",['fac_excel_'+row.id_factura,row.id_factura,row.id_cuenta || before?.id_cuenta || null])).rows[0].id_factura_cliente;
      }
      ...
      if(before && changed.includes('forma_cobro') && !/recibo/i.test(row.forma_cobro) && (await db.query('SELECT 1 FROM prevision_recibos_excel WHERE id_orden=$1 AND id_remesa IS NOT NULL',[row.id_orden])).rowCount)incomeError('La orden '+row.id_orden+' tiene un recibo en remesa; revisa esa asociación antes de cambiar la forma de cobro.');
      const encode=k=>k==='datos_importacion'?JSON.stringify(row[k]):row[k];
      if(!before){const fields=Object.keys(row);await db.query('INSERT INTO ordenes_db('+fields.join(',')+') VALUES('+fields.map((_,i)=>'$'+(i+1)).join(',')+')',fields.map(encode));created++;}
      else if(changed.length){await db.query('UPDATE ordenes_db SET '+changed.map((k,i)=>k+'=$'+(i+1)).join(',')+',updated_at=now() WHERE id_orden=$'+(changed.length+1),[...changed.map(encode),row.id_orden]);updated++;}
      else unchanged++;

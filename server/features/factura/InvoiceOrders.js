...
export async function syncInvoiceOrders(db, invoiceId, actorId = '') {
  const invoice = (await db.query('SELECT * FROM administracion_facturas_clientes WHERE id_factura_cliente=$1', [invoiceId])).rows[0];
  if (!invoice?.id_contrato || invoice.factura_origen_id) return;
  const payments = (await db.query('SELECT * FROM comercial_contratos_cobros WHERE id_contrato=$1 ORDER BY numero_cobro', [invoice.id_contrato])).rows;
  const orderIds = [];
  for (const payment of payments) {
    const matches = (await db.query(`SELECT * FROM tesoreria_ordenes WHERE id_contrato=$1
    ...
    const receipt = previous && (await db.query('SELECT * FROM tesoreria_recibos_importados WHERE id_orden=$1 FOR UPDATE', [previous.id_orden])).rows[0];
    if (previous && changed.some(key => ['cobro_total','forma_cobro','banco_cobro'].includes(key))) {
      const reviewed = (await db.query(`SELECT 1 FROM tesoreria_aplicaciones_cobro a JOIN tesoreria_movimientos_bancarios b USING(id_linea_banco)
    ...
    if (!previous) {
      ...
      await db.query('INSERT INTO tesoreria_ordenes(' + keys.join(',') + ') VALUES(' + keys.map((_,i) => '$' + (i+1)).join(',') + ')', keys.map(key => row[key]));
    } else if (changed.length) {
      await db.query('UPDATE tesoreria_ordenes SET ' + changed.map((key,i) => key + '=$' + (i+1)).join(',') + ',updated_at=now() WHERE id_orden=$' + (changed.length+1), [...changed.map(key => patch[key]), orderId]);
    }
    ...
    if (receipt && !/recibo/i.test(patch.forma_cobro)) {
      await db.query('DELETE FROM tesoreria_recibos_importados WHERE numero_recibo=$1', [receipt.numero_recibo]);

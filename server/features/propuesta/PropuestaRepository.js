...
async function addCuentaProposalComment(client, { idCuenta, idAgente, contenido }) {
  ...
  await client.query(
    `INSERT INTO general_comentarios (id_comentario, id_original_autor, id_last_editor, tipo_entidad, id_entidad, contenido_comentario)
...
async function addProposalSummaryComment(client, propuesta, action = "creada", actorId = "") {
  ...
  const [agentResult, linesResult, paymentsResult] = await Promise.all([
    ...
    client.query(`SELECT producto, medio, publicacion, descripcion_linea, especificaciones_linea, unidades FROM comercial_propuestas_lineas WHERE id_propuesta = $1 ORDER BY numero_linea_propuesta`, [propuesta.id_propuesta]),
    client.query(`SELECT fecha_cobro, importe_cobro, forma_cobro, banco_cobro FROM comercial_propuestas_cobros WHERE id_propuesta = $1 ORDER BY numero_cobro`, [propuesta.id_propuesta]),
  ]);
...
async function assertProposalReadyForAcceptance(client, propuesta) {
  ...
  const { rows } = await client.query(
    `SELECT
       (SELECT COUNT(*)::int FROM comercial_propuestas_lineas WHERE id_propuesta=$1) AS lineas,
       (SELECT COUNT(*)::int FROM comercial_propuestas_cobros WHERE id_propuesta=$1) AS cobros,
       (SELECT COUNT(*)::int FROM comercial_propuestas_cobros
...
async function finalizeProposal(client, propuesta, status, actorId = "") {
  ...
  await client.query(`UPDATE comercial_propuestas_db SET comentarios_adicionales=$1, updated_at=NOW() WHERE id_propuesta=$2`, [comments, propuesta.id_propuesta]);
  ...
  await client.query(
    `UPDATE comercial_contratos SET array_contenidos=$1::jsonb WHERE id_contrato=$2`,
    [JSON.stringify(contentIds.map((idContenido) => ({ id_contenido: idContenido }))), idContrato],
  );

  const proposalPayments = await client.query(
    `SELECT * FROM comercial_propuestas_cobros WHERE id_propuesta=$1 ORDER BY numero_cobro,created_at`,
    [propuesta.id_propuesta],
  );
  const orderIds = [];
  for (const payment of proposalPayments.rows) {
    ...
    await client.query(
      `INSERT INTO comercial_contratos_cobros (
...
async function loadProposalExtras(client, proposalIds) {
  ...
  const [lineasResult, cobrosResult] = await Promise.all([
    client.query(
      `
        SELECT *
        FROM comercial_propuestas_lineas
...
export async function getPropuestas(filters = {}) {
  ...
  const { rows } = await pool.query(
    `
      ...
      FROM comercial_propuestas_db p
      LEFT JOIN comercial_cuentas c ON c.id_cuenta = p.id_cuenta_propuesta
...
export async function getPropuestaById(idPropuesta) {
  ...
  const { rows } = await pool.query(
    `
      ...
      FROM comercial_propuestas_db p
      LEFT JOIN comercial_cuentas c ON c.id_cuenta = p.id_cuenta_propuesta
      LEFT JOIN comercial_contactos ct ON ct.id_contacto = p.id_contacto_propuesta
      LEFT JOIN comercial_contratos con ON con.id_propuesta = p.id_propuesta
...
async function replaceCobros(client, idPropuesta, cobros = []) {
  await client.query("DELETE FROM comercial_propuestas_cobros WHERE id_propuesta = $1", [idPropuesta]);
  for (let index = 0; index < cobros.length; index += 1) {
    const cobro = cobros[index] ?? {};
    await client.query(
      `
        INSERT INTO comercial_propuestas_cobros (
          id_cobro_propuesta,
          id_propuesta,
          numero_cobro,
          fecha_cobro,
...
export async function createPropuesta(payload, actorId = "") {
  ...
  try {
    ...
    await client.query(
      `
        INSERT INTO comercial_propuestas_db (${columns.join(", ")})
...
export async function updatePropuesta(idPropuesta, payload, actorId = "") {
  ...
  try {
    ...
    if (columns.length) {
      ...
      await client.query(
        `
          UPDATE comercial_propuestas_db
    ...
    const { rows: afterRows } = await client.query("SELECT * FROM comercial_propuestas_db WHERE id_propuesta = $1 LIMIT 1", [idPropuesta]);
...
export async function deletePropuesta(idPropuesta, actorId = "") {
  ...
  try {
    ...
    await client.query("DELETE FROM comercial_propuestas_cobros WHERE id_propuesta = $1", [idPropuesta]);
    await client.query("DELETE FROM comercial_propuestas_lineas WHERE id_propuesta = $1", [idPropuesta]);
    const { rows } = await client.query("DELETE FROM comercial_propuestas_db WHERE id_propuesta = $1 RETURNING *", [idPropuesta]);

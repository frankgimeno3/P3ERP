...
export async function getImportedReceipts() {
  return (await getPgPool().query("SELECT r.*,m.importe_total,COALESCE(m.remesa_en_carpeta,r.remesa_en_carpeta) remesa_en_carpeta FROM prevision_recibos_excel r LEFT JOIN prevision_remesas_resumen m ON m.id_remesa=r.id_remesa ORDER BY r.numero_factura,r.numero_cobro")).rows;
}
...
export async function saveReceipt(db, row, actorId = '') {
  const previous = (await db.query('SELECT * FROM prevision_recibos_excel WHERE numero_recibo=$1 FOR UPDATE', [row.numero_recibo])).rows[0];
  let invoice = (await db.query('SELECT * FROM facturas_clientes_db WHERE id_factura_cliente=$1 OR numero_factura=$1', [row.numero_factura])).rows;
  ...
  const accounts = row.cliente ? (await db.query("SELECT id_cuenta FROM cuentas_db WHERE id_cuenta=$1 OR lower(btrim(nombre_empresa))=lower($1) OR lower(btrim(nombre_fiscal))=lower($1)", [row.cliente])).rows : [];
  const account = accounts.length === 1 ? accounts[0].id_cuenta : null;
  if (!invoice.length) invoice = (await db.query("INSERT INTO facturas_clientes_db(id_factura_cliente,numero_factura,id_cuenta,estado) VALUES($1,$2,$3,'en proceso') RETURNING *", ['fac_excel_' + row.numero_factura,row.numero_factura,account])).rows;
  const fact = invoice[0];
  const candidates = previous?.id_orden ? (await db.query('SELECT * FROM ordenes_db WHERE id_orden=$1 FOR UPDATE', [previous.id_orden])).rows
    : (await db.query('SELECT * FROM ordenes_db WHERE (id_factura=$1 OR id_factura=$2) AND numero_cobro=$3 ORDER BY id_orden FOR UPDATE', [fact.id_factura_cliente,row.numero_factura,row.numero_cobro])).rows;
  ...
  if (!order) order = (await db.query("INSERT INTO ordenes_db(id_orden,id_factura,numero_cobro,id_cuenta,forma_cobro,cobro_total,fecha_teorica_cobro,etiqueta_cobro) VALUES($1,$2,$3,$4,'recibo',$5,$6,$7) RETURNING *",
  ...
  const linked = previous || (await db.query('SELECT * FROM prevision_recibos_excel WHERE id_orden=$1 FOR UPDATE', [order.id_orden])).rows[0];
  const remesaId = row.numero_remesa || linked?.id_remesa || null;
  if (linked?.id_remesa && remesaId !== linked.id_remesa && (await db.query('SELECT 1 FROM banco_cobros_ordenes a JOIN lineas_bancos b USING(id_linea_banco) WHERE a.id_orden=$1 AND b.estado_revision LIMIT 1', [order.id_orden])).rowCount) incomeError('Desmarca primero la revisión bancaria para cambiar la remesa del recibo ' + row.numero_recibo + '.');
  if (remesaId) await db.query("INSERT INTO remesas_db(id_remesa,remesa_en_carpeta,importe_declarado) VALUES($1,$2,$3) ON CONFLICT(id_remesa) DO UPDATE SET remesa_en_carpeta=COALESCE(NULLIF(EXCLUDED.remesa_en_carpeta,''),remesas_db.remesa_en_carpeta),importe_declarado=COALESCE(EXCLUDED.importe_declarado,remesas_db.importe_declarado),updated_at=now() WHERE (NULLIF(EXCLUDED.remesa_en_carpeta,'') IS NOT NULL AND EXCLUDED.remesa_en_carpeta IS DISTINCT FROM remesas_db.remesa_en_carpeta) OR (EXCLUDED.importe_declarado IS NOT NULL AND EXCLUDED.importe_declarado IS DISTINCT FROM remesas_db.importe_declarado)",
  ...
  if (linked) {
    ...
    if (changed.length) {
      await db.query('UPDATE prevision_recibos_excel SET ' + changed.map((key,i)=>key+'=$'+(i+1)).join(',') + ',updated_at=now() WHERE numero_recibo=$' + (changed.length+1), [...changed.map(key=>merged[key]),linked.numero_recibo]);
    ...
    await db.query('INSERT INTO prevision_recibos_excel(' + fields.join(',') + ') VALUES(' + fields.map((_,i)=>'$'+(i+1)).join(',') + ')', fields.map(key=>merged[key]));
  ...
  if (changed.length) {
    await db.query('UPDATE ordenes_db SET '+changed.map((key,i)=>key+'=$'+(i+1)).join(',')+',updated_at=now() WHERE id_orden=$'+(changed.length+1),[...changed.map(key=>changes[key]),order.id_orden]);

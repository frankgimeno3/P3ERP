...
export async function getIngresosAdicionales(tipo = "") {
  ...
  const values = [];
  const where = "WHERE NOT EXISTS(SELECT 1 FROM tesoreria_ordenes o WHERE o.id_orden=ia.id_ingreso_adicional)" + (tipo && tipo !== "todos" ? " AND ia.tipo_ingreso = $1" : "");
  if (tipo && tipo !== "todos") values.push(tipo === "transfers" ? "transferencia" : "recibo");
  const { rows } = await pool.query(`
    SELECT ia.*, c.nombre_empresa
    FROM tesoreria_ingresos_adicionales ia
    LEFT JOIN comercial_cuentas c ON c.id_cuenta = ia.id_cuenta
    ${where}
...
export async function ensureAdditionalOrder(db, row, actorId='') {
  if((await db.query('SELECT 1 FROM tesoreria_ordenes WHERE id_orden=$1',[row.id_ingreso_adicional])).rowCount)return;
  let invoice=null;

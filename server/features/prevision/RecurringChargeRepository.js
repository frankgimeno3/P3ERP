...
export async function insertRecurringCharge(db, body, reusePayroll = false) {
  ...
  return (await db.query('INSERT INTO tesoreria_cargos_recurrentes(tipo_cargo,id_proveedor,id_agente,tipo_programacion,programacion) VALUES($1,$2,$3,$4,$5::jsonb) RETURNING *', [data.tipo_cargo, data.id_proveedor, data.id_agente, data.tipo_programacion, JSON.stringify(data.programacion)])).rows[0];
}
...
export async function deleteRecurringCharge(id,body) {
  ...
  try {
    ...
    const row=(await db.query('SELECT * FROM tesoreria_cargos_recurrentes WHERE id_cargo_recurrente=$1 FOR UPDATE',[id])).rows[0];
    if(!row)throw new RecurringChargeError('Cargo previsto no encontrado.',404);
...
export async function updateRecurringCharge(id,body) {
  ...
  try {
    ...
    const result=(await db.query('UPDATE tesoreria_cargos_recurrentes SET tipo_programacion=$2,programacion=$3::jsonb,updated_at=now() WHERE id_cargo_recurrente=$1 RETURNING *',[id,data.tipo_programacion,JSON.stringify(data.programacion)])).rows[0];

...
export async function ensureOrderReceipt(db, orderId, actorId = '') {
  ...
  const result = await db.query(`INSERT INTO tesoreria_recibos_importados
    ...
    WHERE tesoreria_recibos_importados.id_orden IS NULL RETURNING *`,
...
export async function reconcileBankIncome(db, line, item, actorId = '') {
  ...
  const old = (await db.query('SELECT id_orden FROM tesoreria_aplicaciones_cobro WHERE id_linea_banco=$1', [line.id_linea_banco])).rows;
  let orders = [];
  if (item.incomeType === 'remesa') {
    ...
    await db.query('SELECT id_remesa FROM tesoreria_remesas WHERE id_remesa=ANY($1::text[]) ORDER BY id_remesa FOR UPDATE', [ids]);
  ...
  await db.query('DELETE FROM tesoreria_aplicaciones_cobro WHERE id_linea_banco=$1', [line.id_linea_banco]);

...
export async function reviewPayrollMovement(id, body) {
  ...
  try {
    ...
    const line = (await db.query('SELECT * FROM tesoreria_movimientos_bancarios WHERE id_linea_banco=$1 FOR UPDATE', [id])).rows[0];
    ...
    const linked = await db.query('SELECT id_empleado FROM laboral_nominas WHERE id_transferencia=$1 UNION ALL SELECT id_empleado FROM laboral_anticipos WHERE id_transferencia=$1', [id]);
    ...
    const saved = (await db.query('UPDATE tesoreria_movimientos_bancarios SET id_agente=$1,id_proveedor=NULL,id_cuenta=NULL,estado_revision=TRUE,comentarios=COALESCE($2,comentarios),nomina_revision=$3::jsonb,updated_at=NOW() WHERE id_linea_banco=$4 RETURNING *', [body.id_agente,body.comentarios,JSON.stringify(snapshot),id])).rows[0];

...
const paymentTable = kind => kind === 'nominas' ? 'laboral_nominas' : 'laboral_anticipos';
...
export async function getPayment(kind, id) {
  ...
  const advances = kind === 'nominas' ? (await db.query('SELECT * FROM laboral_anticipos WHERE id=ANY($1::text[]) ORDER BY created_at', [payment.anticipos])).rows : [];
  const transferIds = [payment.id_transferencia, ...advances.map(a => a.id_transferencia)].filter(Boolean);
  const transfers = transferIds.length ? (await db.query('SELECT * FROM tesoreria_movimientos_bancarios WHERE id_linea_banco=ANY($1::text[])', [transferIds])).rows : [];
...
async function checkTransfer(db, transferId, employeeId, kind, id) {
  ...
  const bank = await one(db, 'SELECT * FROM tesoreria_movimientos_bancarios WHERE id_linea_banco=$1 FOR UPDATE', [transferId]);
  if (Number(bank.importe) >= 0) throw new LaboralError('Selecciona un movimiento de salida para la transferencia.');
  const ownCharge = bank.id_cargo_recurrente && (await db.query("SELECT 1 FROM tesoreria_cargos_recurrentes WHERE id_cargo_recurrente=$1 AND tipo_cargo='nomina' AND id_agente=$2", [bank.id_cargo_recurrente,employeeId])).rowCount;
  ...
  const used = await db.query("SELECT id FROM laboral_nominas WHERE id_transferencia=$1 AND NOT ($2='nominas' AND id=$3) UNION ALL SELECT id FROM laboral_anticipos WHERE id_transferencia=$1 AND NOT ($2='anticipos' AND id=$3)", [transferId, kind, id]);
...
async function syncAdvances(db, employeeId, year, month) {
  const { rows } = await db.query('SELECT id,importe_neto,estado FROM laboral_anticipos WHERE id_empleado=$1 AND anio=$2 AND mes=$3 ORDER BY created_at', [employeeId, year, month]);
  const { rows: payrolls } = await db.query('SELECT * FROM laboral_nominas WHERE id_empleado=$1 AND anio=$2 AND mes=$3 FOR UPDATE', [employeeId, year, month]);
  ...
  await db.query('UPDATE laboral_nominas SET anticipos=$1,updated_at=NOW() WHERE id=$2', [rows.map(a => a.id), payrolls[0].id]);
}
...
export async function listTransfers(employeeId) {
  return (await getPgPool().query(`SELECT b.id_linea_banco,b.banco,b.fecha_valor,b.concepto,b.importe FROM tesoreria_movimientos_bancarios b
    WHERE b.importe<0 AND b.id_proveedor IS NULL AND b.id_cuenta IS NULL AND b.id_pago IS NULL AND b.id_orden IS NULL
      AND (b.id_cargo_recurrente IS NULL OR EXISTS (SELECT 1 FROM tesoreria_cargos_recurrentes cr WHERE cr.id_cargo_recurrente=b.id_cargo_recurrente AND cr.tipo_cargo='nomina' AND cr.id_agente=$1))
      AND (b.id_agente IS NULL OR b.id_agente=$1)
    ORDER BY b.created_at DESC`, [employeeId || ''])).rows;
}
export async function listCalendars() {
  return (await getPgPool().query('SELECT c.anio,COUNT(e.id)::int AS eventos FROM laboral_calendarios c LEFT JOIN laboral_eventos_calendario e ON e.anio=c.anio GROUP BY c.anio ORDER BY c.anio DESC')).rows;
}
export async function createCalendar(body) {
  const { anio } = validate('calendario', body);
  return one(getPgPool(), 'INSERT INTO laboral_calendarios(anio) VALUES ($1) ON CONFLICT(anio) DO UPDATE SET anio=EXCLUDED.anio RETURNING *', [anio]);
}
export async function getCalendar(year) {
  const { anio } = validate('calendario', { anio: year });
  await one(getPgPool(), 'SELECT anio FROM laboral_calendarios WHERE anio=$1', [anio]);
  return { anio, eventos: (await getPgPool().query('SELECT e.*,e.inicio::text,e.fin::text FROM laboral_eventos_calendario e WHERE anio=$1 ORDER BY e.inicio,e.titulo', [anio])).rows };
}
export async function saveEvent(id, body) {
  ...
  return transaction(async client => {
    if (id) await one(client, 'SELECT id FROM laboral_eventos_calendario WHERE id=$1 FOR UPDATE', [id]);
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      await client.query(id && i === 0 ? 'UPDATE laboral_eventos_calendario SET anio=$1,tipo=$2,titulo=$3,inicio=$4,fin=$5,comentarios=$6 WHERE id=$7' : 'INSERT INTO laboral_eventos_calendario(anio,tipo,titulo,inicio,fin,comentarios,id) VALUES ($1,$2,$3,$4,$5,$6,$7)', values);
    }
    return { id: firstId };
  });
}
...
export async function getEmployee(id, year) {
  ...
  const [libres, ausencias, comentarios] = await Promise.all([
    db.query('SELECT numero,fecha::text FROM laboral_dias_libre_disposicion WHERE id_empleado=$1 AND anio=$2 ORDER BY numero', [id, anio]),
    db.query('SELECT a.*,a.inicio::text,a.fin::text FROM laboral_ausencias a WHERE id_empleado=$1 AND inicio <= make_date($2,12,31) AND fin >= make_date($2,1,1) ORDER BY a.inicio DESC', [id, anio]),
    db.query('SELECT * FROM laboral_comentarios_empleados WHERE id_empleado=$1 ORDER BY created_at DESC', [id]),
  ]);
...
export async function saveFreeDays(id, body) {
  ...
  return transaction(async db => {
    ...
    await db.query('DELETE FROM laboral_dias_libre_disposicion WHERE id_empleado=$1 AND anio=$2', [id,d.anio]);
    for (const day of d.fechas) await db.query('INSERT INTO laboral_dias_libre_disposicion(id_empleado,anio,numero,fecha) VALUES ($1,$2,$3,$4)', [id,d.anio,day.numero,day.fecha]);
    return { ok: true };
  });
}
export async function saveAbsence(employeeId, id, body) {
  const d = validate('ausencias', body);
  return one(getPgPool(), id ? 'UPDATE laboral_ausencias SET tipo=$1,inicio=$2,fin=$3,comentarios=$4 WHERE id_empleado=$5 AND id=$6 RETURNING id' : 'INSERT INTO laboral_ausencias(tipo,inicio,fin,comentarios,id_empleado,id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id', [d.tipo,d.inicio,d.fin,d.comentarios,employeeId,id || randomUUID()]);
}
export async function addComment(employeeId, body) {
  const d = validate('comentarios', body);
  return one(getPgPool(), 'INSERT INTO laboral_comentarios_empleados(id,id_empleado,comentario) VALUES ($1,$2,$3) RETURNING *', [randomUUID(),employeeId,d.comentario]);
}
export async function listProcesses() {
  return (await getPgPool().query('SELECT p.*,COUNT(c.id)::int AS candidatos FROM laboral_procesos_seleccion p LEFT JOIN laboral_candidatos c ON c.id_proceso=p.id GROUP BY p.id ORDER BY p.created_at DESC')).rows;
}
export async function getProcess(id) {
  const db = getPgPool(), process = await one(db, 'SELECT * FROM laboral_procesos_seleccion WHERE id=$1', [id]);
  return { ...process, candidatos: (await db.query('SELECT * FROM laboral_candidatos WHERE id_proceso=$1 ORDER BY created_at,nombre', [id])).rows };
}
export async function saveProcess(id, body) {
  const d = validate('procesos', body);
  return one(getPgPool(), id ? 'UPDATE laboral_procesos_seleccion SET nombre=$1,oferta_condiciones=$2,mensaje_pre_llamada=$3,mensaje_post_llamada=$4,mensaje_rechazo=$5,updated_at=NOW() WHERE id=$6 RETURNING id' : 'INSERT INTO laboral_procesos_seleccion(nombre,oferta_condiciones,mensaje_pre_llamada,mensaje_post_llamada,mensaje_rechazo,id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id', [d.nombre,d.oferta_condiciones,d.mensaje_pre_llamada,d.mensaje_post_llamada,d.mensaje_rechazo,id || randomUUID()]);
}
export async function saveCandidate(processId, id, body) {
  const d = validate('candidatos', body);
  return one(getPgPool(), id ? 'UPDATE laboral_candidatos SET nombre=$1,resumen_cv=$2,comentarios=$3,estado=$4,updated_at=NOW() WHERE id_proceso=$5 AND id=$6 RETURNING id' : 'INSERT INTO laboral_candidatos(nombre,resumen_cv,comentarios,estado,id_proceso,id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id', [d.nombre,d.resumen_cv,d.comentarios,d.estado,processId,id || randomUUID()]);
}
export async function deleteRecord(kind, id, parentId) {
  const tables = { eventos: ['laboral_eventos_calendario', null], ausencias: ['laboral_ausencias','id_empleado'], comentarios: ['laboral_comentarios_empleados','id_empleado'], candidatos: ['laboral_candidatos','id_proceso'] };
...
export const documentOwners = { empleados: ['id_empleado','agentes_db','id_agente'], nominas: ['id_nomina','laboral_nominas','id'], anticipos: ['id_anticipo','laboral_anticipos','id'] };
...
export async function listDocuments(kind, id) {
  ...
  return (await getPgPool().query(`SELECT id,nombre,content_type,tamano,created_at FROM laboral_documentos WHERE ${column}=$1 ORDER BY created_at DESC`, [id])).rows;
}
export async function addDocument(kind, ownerId, data) {
  const column = await documentOwner(kind, ownerId);
  return one(getPgPool(), `INSERT INTO laboral_documentos(id,${column},nombre,content_type,tamano,s3_key,contenido) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id,nombre`, [data.id,ownerId,data.nombre,data.contentType,data.size,data.key || null,data.contenido || null]);
}
export async function getDocument(id) { return one(getPgPool(), 'SELECT * FROM laboral_documentos WHERE id=$1', [id]); }

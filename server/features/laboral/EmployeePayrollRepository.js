...
export async function employeePayrolls(id, employee) {
  ...
  const rows = (await db.query(`SELECT n.*, COALESCE(NULLIF(a.nombre_completo_agente,''),
    ...
    AND EXISTS(SELECT 1 FROM cargos_recurrentes c WHERE c.tipo_cargo='nomina' AND c.id_agente=n.id_empleado)
  ...
  result.movimientos = (await db.query(`SELECT l.*, ae.id AS id_anticipo,
    ...
    FROM lineas_bancos l JOIN cargos_recurrentes c USING(id_cargo_recurrente)
    LEFT JOIN anticipos_empleados ae ON ae.id_transferencia=l.id_linea_banco
    WHERE c.tipo_cargo='nomina' AND c.id_agente=$1 ORDER BY l.created_at DESC,l.id_linea_banco`, [result.id_empleado])).rows;

...
export async function employeePayrolls(id, employee) {
  ...
  const rows = (await db.query(`SELECT n.*, COALESCE(NULLIF(a.nombre_completo_agente,''),
    ...
    (SELECT count(*)::int FROM tesoreria_cargos_recurrentes c WHERE c.tipo_cargo='nomina' AND c.id_agente=n.id_empleado) AS cargos
    FROM laboral_empleados_en_nomina n JOIN agentes_db a ON a.id_agente=n.id_empleado
  ...
  const result = rows[0];
  result.cargos = (await db.query("SELECT * FROM tesoreria_cargos_recurrentes WHERE tipo_cargo='nomina' AND id_agente=$1 ORDER BY created_at DESC", [result.id_empleado])).rows;
  result.movimientos = (await db.query(`SELECT l.*, ae.id AS id_anticipo,
    ...
    FROM tesoreria_movimientos_bancarios l LEFT JOIN tesoreria_cargos_recurrentes c USING(id_cargo_recurrente)
    LEFT JOIN laboral_anticipos ae ON ae.id_transferencia=l.id_linea_banco
    WHERE (c.tipo_cargo='nomina' AND c.id_agente=$1) OR ae.id_empleado=$1
      OR (l.id_agente=$1 AND l.nomina_revision->>'decision'='otros')
      OR EXISTS(SELECT 1 FROM laboral_nominas p WHERE p.id_empleado=$1 AND p.id_transferencia=l.id_linea_banco)
    ORDER BY l.created_at DESC,l.id_linea_banco`, [result.id_empleado])).rows;
  result.periodos = (await db.query('SELECT * FROM laboral_nominas WHERE id_empleado=$1 ORDER BY anio DESC,mes DESC',[result.id_empleado])).rows;
  result.anticipos = (await db.query('SELECT * FROM laboral_anticipos WHERE id_empleado=$1 ORDER BY anio DESC,mes DESC,created_at DESC',[result.id_empleado])).rows;
  return result;
}
...
export async function saveAgentTask(id, body) {
  ...
  const db = getPgPool();
  const result = id ? await db.query(`UPDATE laboral_tareas_empleado SET nombre=$2,estado=$3,descripcion=$4,updated_at=now() WHERE id=$1 AND agente=$5 RETURNING *`,[id,nombre,body.estado,descripcion,body.agente])
    : await db.query(`INSERT INTO laboral_tareas_empleado(id,nombre,estado,descripcion,agente) VALUES($1,$2,$3,$4,$5) RETURNING *`,[`tarea_${randomUUID().replaceAll('-','')}`,nombre,body.estado,descripcion,body.agente]);
